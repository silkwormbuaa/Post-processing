...
dataset  = tp.data.load_tecplot("valley_w.dat")
dataset  = tp.data.load_tecplot("valley_2.dat")
...
tau      = np.multiply(mu2,np.divide(du,dy))/d_p

with open("Cf_valley.dat",'w') as f:
    for i in range(len(x1)):
        f.write(str('%.7f'%x1[i])+' '+str('%.7f'%tau[i])+'\n')
