...
def main():
    ...
    clipbox   = [-20, 10, 0, 6, -1, 1]
    
    dataset = data_preparation( case_dir, vars_read, vars_out, rescale )
    dataset = dataset.clip_box( clipbox, invert=False )
    
    pv_visualize( dataset, 'u`v`', clipbox )
...
def pv_visualize( dataset, varname, clipbox ):
    ...
    ax.imshow(image, extent=clipbox[:4])
