...
# -*- coding: utf-8 -*-
'''
@File    :   snap_isosurface_pfluc.py
@Time    :   2025/05/14 
@Author  :   Wencan WU 
@Version :   1.0
@Email   :   w.wu-3@tudelft.nl
@Desc    :   Show the isosurface of pressure fluctuation
'''
...
bbox      = [-35.0, 35.0, -1.3, 31, -999.0, 999.0]
vars_in   = ['u','p','T']
vars_out  = ['u','p','T', 'p_fluc']
...
out_dir    = dirs.pp_snp_sl_iso + '/figs'
...
if mpi.rank == 0:
    ...
    stat.read_statistic(block_list=blocklist, vars_in=['p'])
...
if roughwall: vars_out += ['p_fluc','mu','wd']
else:         vars_out += ['p_fluc','mu']
...
os.chdir( out_dir )
clock = timer("show pressure fluctuation isosurface near pressure fluctuation max:")

def show_isosurface( snapfile ):
    ...
    for bl in snap.snap_data:
        if bl.num in blocklist:
            ...
            bl.df['p_fluc'] = (bl.df['p'] - stat.bl[stat.bl_nums.index(bl.num)].df['p'])/p_ref
            
    itstep  = snap.itstep
    itime   = snap.itime
    figname = f'p_fluc_{itstep:08d}.png'
    ...
    bubble_pos = point_data.contour( [0.12], scalars='p_fluc' )
    bubble_pos.set_active_scalars('p_fluc')
    
    bubble_neg = point_data.contour( [-0.12], scalars='p_fluc' )
    bubble_neg.set_active_scalars('p_fluc')
    ...
    p.add_mesh( bubble_pos, cmap=cmap, clim=[-0.30,0.30], show_scalar_bar=True,
                lighting=True)
    p.add_mesh( bubble_neg, cmap=cmap, clim=[-0.30,0.30], show_scalar_bar=False,
                lighting=True)
    
    camera_pos = [(40.25,39.78,60.0),(10.27,0.0,0.0),(-0.21,0.86,-0.47)]
    ...
    if mpi.rank == 0:
        ...
        while True:
            ...
            if task_index is None: break
                ...
                show_isosurface( snapfiles[task_index] )
