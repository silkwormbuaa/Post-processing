...
# -*- coding: utf-8 -*-
'''
@File    :   relative_velocity.py
@Time    :   2024/11/04 
@Author  :   Wencan WU 
@Version :   1.0
@Email   :   w.wu-3@tudelft.nl
@Desc    :   show relative velocity at the shear layer of separation bubble.
'''
...
vars_in  = ['u', 'v', 'w','p','T']
vars_out = ['u_r', 'w3','v_r']
labels   = [r"$u_{r}/u_{\infty}$", r'$\omega$', r"$v_{r}/u_{\infty}$" ]
colmaps  = ['coolwarm',        'coolwarm', 'coolwarm']
ranges   = [[-0.2,0.2],        [-1.0,1.0], [-0.2,0.2]]
...
if mpi.is_root:
    ...
    statz.read_statistic(block_list=blocklist, vars_in=['u','v','w','p'])
...
# -- read in the snapshot and compute density gradient
    ...
    snap.compute_gradients( grads=['vorticity'] )
