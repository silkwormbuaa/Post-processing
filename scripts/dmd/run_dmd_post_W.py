...
with timer("\nShow snapshots"):
    ...
    zmin = modes_temp.df_modes['z'].min()
    zmax = modes_temp.df_modes['z'].max()
    
    x_i = np.linspace( xmin, xmax, 451)
    z_i = np.linspace( zmin, zmax, 121)
    
    modes_temp.grids_interp = np.meshgrid( x_i, z_i )
    
    
    # plot reconstructed data
    
    xx,zz,v = modes_temp.interp_recons( 0 )

    plot_dmd_mode( (xx,zz), v, 
    ...
    xx,zz,v = modes_temp.interp_mode( indx_alpha_max )
    ...
    for n_mode in range( n_modes//2 + 1 ):
        ...
        for i, phase in enumerate(phases):
            ...
            xx,zz,v = modes_temp.interp_mode( n_mode, phase=phase )
